...
class Figure:
    ...
    def __init__(self, color, *sides):
        ...
        self.__color = color  # Цвет в формате RGB
        self.__sides = sides  # Список сторон (целые числа)
        self.filled = False        #Закрашенный, bool
        self.__sides = []
        self.set_sides(*sides)
    def get_color(self): #Возвращает цвет фигуре
        return self.__color

    def __is_valid_color(self, r, g, b): #проверка на корректность

        if 0 < r <= 255 and 0 < g <= 255 and 0 < b <= 255:  # Цвет корректный
            return r, g, b
        else:
            return self.__color  # Возвращает текущий цвет, если новый некорректный
    ...
    def set_sides(self, *sides): #Установка стороны фигуры
    ...
    def __len__(self): #Возвращает сумму длин всех сторон
...
class Cube(Figure):
    ...
    def get_volume(self): #вычисляет объем куба
...
class Triangle(Figure):
    ...
    def __init__(self, color, *sides): #высота треугольника по формуле Герона
        super().__init__(color, *sides)
        self.sides = sides
...
circle1 = Circle((200, 200, 100), 10) # Цвет и радиус
